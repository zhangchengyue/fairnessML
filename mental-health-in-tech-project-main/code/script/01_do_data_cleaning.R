...
# Get helper functions for data cleaning
...
# Function "CleanGenderVar" is sourced from funs_do_data_cleaning.R file.
df$gender <- CleanGenderVar(df$gender)

unique(df$gender) # [1] "Male"   "Female" "Other" 
...
df <- SeparateDx(df, df$mh_dx_specify) # Function "SeparateDx" is sourced from funs_do_data_cleaning.R file.

df$mh_maybe_specify <- strsplit(df$mh_maybe_specify, "\\|")
df$mh_maybe_specify <- gsub(" ", "", df$mh_maybe_specify)
df <- SeparateNotDx(df, df$mh_maybe_specify) # Function "SeparateNotDx" is sourced from funs_do_data_cleaning.R file.
...
#company-employed
# Function "DataSummary" is sourced from funs_do_data_cleaning.R file.
